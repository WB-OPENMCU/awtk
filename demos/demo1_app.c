...
ret_t application_init() {
  ...
  ok = button_create(win, 5, 5, 70, 30);
  widget_set_text(ok, L"Inc");

  cancel = button_create(win, 80, 5, 70, 30);
  widget_set_text(cancel, L"Dec");

  show_dialog = button_create(win, 155, 5, 70, 30);
...
//  image_set_draw_type(image, IMAGE_DRAW_SCALE);
  ...
  label = label_create(win, 5, 40, 70, 30);
  widget_set_text(label, L"Left");
  widget_use_style(label, "left");

  label = label_create(win, 80, 40, 70, 30);
...
//#endif
  ...
  label = label_create(win, 155, 40, 70, 30);
...
//  slider_set_vertical(slider, TRUE);
  ...
  check_button = check_button_create(win, 10, 120, 80, 30);
  widget_set_text(check_button, L"Book");

  check_button = check_button_create(win, 100, 120, 80, 30);
  widget_set_text(check_button, L"Food");

  radio_button = check_button_create_radio(win, 10, 160, 80, 30);
  widget_set_text(radio_button, L"Book");

  radio_button = check_button_create_radio(win, 100, 160, 80, 30);
  widget_set_text(radio_button, L"Food");

  radio_button = check_button_create_radio(win, 10, 190, 50, 30);
  widget_set_text(radio_button, L"Pencil");
  widget_set_value(radio_button, TRUE);



  group_box = group_box_create(win, 10, 230, 32 * 3, 32);
  ...
  check_button = check_button_create(win, 108, 230, 32, 32);
